...
Separate into clear sections. Keep in mind we have two hours total.

:00  Intro to Qualtrics and labelled data; reading in (up to The Plan)
:15  Selection helpers, renaming, using labelled data
:40  Practice time and break
1:00 Data dictionary, conversion with `across()`, comp scores with `c_across()`
1:25 Practice time
1:50 End/Break
2:00 Optional: download data from Qualtrics, get started working with it
...
This tutorial was co-written with [Dr Dan Evans](https://drdanielleevans.netlify.app/), drawing on her existing resources for Qualtrics and her extensive experience support dissertation students.
...
mil_data2 <- here::here("data/mil_data.sav") %>% haven::read_sav()

labelled::var_label(mil_data2$StartDate) <- "Date and time questionnaire was started"

labelled::var_label(mil_data2$StartDate)

```


::: {.callout-warning title="HaRd Mode: Using Regular Expressions" collapse="true"}

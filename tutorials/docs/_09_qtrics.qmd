...
In this section we'll have a quick look at how to set up Qualtrics to work as smoothly as possible with R. This is also covered in a [QQM Skills Lab](https://drdanielleevans.netlify.app/posts/qqm_22/skills_lab_05-1).
...
::: callout-tip
It is **strongly recommended** that you rename each question something informative when you create it. We will cover [renaming in the dataset in depth below](#renaming) but it's much better to avoid having to do this in the first place by naming your variables something sensible to begin with. The less you have to fiddle with your data, the better. 
...
This option still involves some hidden, manual operations that can introduce errors, so let's look at an alternative. 
...
If you'd like to edit labels, you can do it "manually" - that is, just writing a whole new label from scratch in R.
...
::: {.repro-icon}
:::
RepRoducibility: Regex Relabelling 
:::

::: {.repro-body}
Now. Why, you might wonder, is all the faff with regex *better*?

Well, it might not be. You might find it more frustrating or effortful to generate the right regex pattern than to replace the label "manually", and in that case, there's nothing wrong with just writing out the label you want as long as you do in R and there is a record of it.  
...
Second, data dictionaries are hugely useful for other people. This would be a massive help to, for example, a colleague who may be assisting you with your data analysis, or checking [the reproducibility of your analysis](https://osf.io/d26jv). If you want to share your data publicly, including a dictionary/codebook is not only a kindness to other users but also helps prevent misuse or misunderstandings.
:::

...
In the last line creating `sticsa_trait_se`, both `sticsa_trait_sd` and `n` are variables created previously in the same `summarise()` command.
...
This second summary table, grouped by both MCQ and gender, would likely be easier to read with one variable on separate rows and the other in separate columns. If you're interested in this, run `vignette("pivot")` in the Console.
:::
...
1.  Create an object (here `fxs` short for "functions") that contains a list with the following elements:
2.  Each element is named on the LHS and contains a formula on the RHS. Here the formulae are named the same as their functions, but this doesn't have to be the case.
...
5.  Apply the list of functions stored in the `fxs` object.
...
Within `across()`, we are building on what we've seen before with this function. The first argument selects which variables to use using `<tidyselect>`. In this case, I've selected all of the variables that contain "score". The second argument provides a list of function(s) to apply to all of the selected variables, which here is the `fxs` object.
...
As a final note here, the creation of the `fxs` object containing a list of functions isn't technically necessary - you can just provide the list in `across` directly.
...
For the last task, capitalising the values in the MCQ column, we'll have a first, brief brush with text string manipulation. Working with strings is well outside the scope of this tutorial[^regex], but for simple operations, this is a good opportunity to meet the {stringr} package.
...
Next, let's turn to `pattern = "^(.)"`. This might look like a keysmash, but it has two elements:

  - `^` is the symbol to indicate the start of a string.
...
As a final example of 2, I could change this command to change the first character of each string with any other character I'd like:
...
This lengthy and complicated optional aside is included for demonstration purposes only, and is well beyond what we have or will covered in this tutorials.
...
So there you have it: a (mostly) fully automated and nicely formatted table requiring no hard-coding of specific names or variables, at the bargain price of a few precious hours of my life and a not-insubstantial chunk of my sanity. You're welcome!
:::
:::

### Exporting and Linking

If you are writing your work exterior to Quarto/your code, you will instead need export your summary and then import it into the new document. However, it's essential to keep a close eye on changes or updates, so that tables don't get out of date if the source data is updated.

#### Creating a Folder

To save our plot, we'll first need to create a folder to save our summary tables in. You can (and likely will!) do this manually, by clicking. Both Posit Cloud and RStudio Desktop have a + Folder button (the first one in the Files tab), which will create a new folder in whatever folder you are currently looking at. Make sure you are in the project folder, then enter the folder name and click OK.
...
First, you will need to export the summary tibble as a .csv file. This is a reminder of what we covered previously about exporting datasets.

```{r}
#| eval: false

readr::write_csv(anx_scores_sum,
                 file = here::here)
```
...
Then, pick a single pair, generate the correlation output, and `report()` the result.

::: {.callout-note collapse="true" title="Solution"}

To start, let's do the same as above, just replacing the string in `contains()`:
...
Yikes, that's a lot of pairwise comparisons - and they're all significant, hooray. I'm going to pick one of the

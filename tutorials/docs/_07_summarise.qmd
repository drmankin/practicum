...
Hardworking, versatile, and indispensable, `mutate()` makes changes within a given dataset by creating new variables (columns) whereas `summarise()` uses the information in a given dataset to create a new, separate summary dataset.
...
::: {.callout-note title="Solution" collapse="true"}
Under "Arguments", the help documentation describes the `.names` argument, which allows us to easily create new variable names. This uses a "glue specification" (see [the {glue} package](https://glue.tidyverse.org/) for more on that!) but we don't need much more than what's in the help documentation for this.

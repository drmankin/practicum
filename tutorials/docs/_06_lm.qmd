...
There are two key ways you might want to write and render a Quarto document for your analysis.
...
[^1]: Mostly true, except for [inline code](#inline-code)!
...
<!-- 1. automatic with `report()`
     - Don't blindly use it without checking!!
     - Look at output with summary() and use report() to SUPPLEMENT
     - Compare to make sure it's correct
2. inline with papaja
3. fully custom with inline code and subsetting
--> 
...
This is a very easy and fast way to get APA-formatted results that you can just put into your paper. AND the output contains the inferential conclusions that can be drawn from the results, based on the popular decision cut-off of p < .05 (more on this later!). The results also look much nicer than the output provided by `summary()` which can make it tempting to fit your model and go straight to reporting the results with `report()`. However, `R`, `RStudio` and all the packages in existence get frequent updates which may or may not lead to changes in how a particular function works. Base `R` is always the safest way to interact with and examine your data, so even if you are planning to use `report()` or any other function to beautify your results section, it is best to check that the results match the results produced by `summary()`. Using different methods to do the same thing has helped me catch errors I would have otherwise missed numerous times. 

::: {.callout-tip title="MoRe About: report()" collapse="true"}
Above, we used the `report()` to summarise the results of our linear models but this is not all that it does! `report()` is very versatile and it will give you a different output depending on the type of object that you give it is input. For example, we can also use this function to output overall descriptive of a given variable: 
...
<!-- INTRO TO INLINE CODING HERE -->

*Inline code* is a small bit of R code written in the body text ("inline") that will, when the document is rendered, automatically insert the right output. We briefly encountered inline code already [Tutorial 04](04_filter.qmd#reporting). Inline code is written as follows, in the text: `` `r knitr::inline_expr("some_r_code")` ``. Note the backticks; these are NOT apostrophes, and are typically located on the top left of a UK keyboard to the lefter of the 1/! key. We will look into inline code in more detail later.
...
We've got a huge number of options here, but for this exercise we wanted R^2^, *F*, and *p*. All three are given under `$full_result$modelfit$r2` and we can access these results with [subsetting](03_datasets.qmd). We will need to save the output from `apa_print()` into an object, then we can subset it using inline code:
...
<!-- UPDATE: Takes too long - use report() as main example and just talk through inline code and exercises for people to do later if they want (so rearrange this section) -->

Our final option for reporting results is to use fully custom inline code to report each of our numerical results independently. This is the most flexible option for reporting results because it allows you to put a single result anywhere you like. But first, to be able to refer to a specific result in our output, we need to know what information we have available.

::: callout-tip
This section will make extensive use of `$` subsetting, which was covered in [Tutorial 03](03_datasets.qmd), and `[]` subsetting, which was covered in [Tutorial 01](01_intro.qmd). Getting comfortable with subsetting is valuable not only for reporting results but also for conducting additional analyses on the results of some computation. Even though we are focusing on inline code today, this is just and example and subsetting is useful in coding more generally. 
:::

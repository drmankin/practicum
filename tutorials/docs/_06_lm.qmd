...
You may have noticed that the output we've seen so for only contains *un*standardised model parameter estimates. If we want standardised *B*s expressing the relationship between predictor(s) and outcome in standard deviation units, we can make use of the `model_parameters()` function from the {parameters} package to standardise our *b*s. To obtain standardised *b*s, we can use the `standarize = "refit"` argument in the `parameters::model_parameters()` function,
...
```{r}
## Fit the same model using the lmRob function
eval_lm_rob <- robust::lmRob(eval ~ beauty + gender, data = teach_tib)

## Print out the model parameteres
...
To test and adjust for heteroscedastic residuals, we can re-estimate the standard error using a robust method. To do this, we'll use the `parameters::model_parameters()` function with the argument `vcov = "HC4"` as recommended in the {discovr} tutorial, using the unadjusted model object `eval_full_lm` as the first argument.
...
::: {.callout-note collapse="true" title="Solution"}
<!--Note from Reny: When loading the data from the URL I had to chagne syn_sequence_space to syn_seq_space--->
```{r}
syn_full_lm <- lm(scsq_imagery ~ scsq_techspace + syn + scsq_language, data = syn_data)
syn_inter_lm <- lm(scsq_imagery ~ syn_graph_col*syn_sequence_space, data = syn_data)
...
<!-- RENY - This bit onwards is all about using dynamic reporting in Quarto. Feel free to delete wholesale unless you want to use any of it! -->
...
3.  The results from the code should exactly match the results in the paper.
...
This point is tricky because each function in R will have it's own quirks or properties you need to be aware of. Just as an example, since we are doing linear models today, we will look into how data types can affect the results produced by the `lm()` function.

In some datasets, categorical variables are sometimes coded with numerical values. For example, in an experiment where you have a control group and an experimental group, the you may code the control group as a 0, and the experimental group as a 1. Before you do any analyses, you need to first make sure that these variables are of a factor type rather than a numeric type. If the variable has only 2 levels, nothing bad will happen, but if it has more than 2 levels, the results from your analysis won't be meaningful.
...
# 1 – evening, 2 – morning, 3 – lunch time
teach_tib <-  teach_tib |> 
  dplyr::mutate(class_time = sample(c(1, 2, 3), size = nrow(teach_tib), replace = TRUE))
...
The interpretation of these results is that as class time increases by 1 unit, evaluations changes by `r round(summary(class_time_lm)$coefficients[2, 1], 2)` points. Class time is however not a continuous numeric variable, so this doesn't make sense. Instead, we have to change class time to a categorical variable:
...
By default, R will use the first level of the categorical variable as the value for the intercept. Then, it creates two coefficients -- one showing the difference between levels 1 and 2, and the other showing the different between levels 1 and 3. You can explicitly define the contrasts you want to compute, but we won't be looking into this here.
...
::: callout-warning
Before you proceed, make sure you have completed and saved your fist worksheet with all of your code!
:::
...
To prepare for our analysis, we will need a place to load packages and read in the data. Any executable code - that is, code that you want to run and do something - must be written in a code chunk and NOT in the body text[^1].

[^1]: Mostly true, except for [inline code](04_lm.qmd#inline-code)!
...
### The results from the code should exactly match the results in the paper
...
The paragraph from the previous exercise was very clearly missing the actual statistical results of the linear model analysis. We *could* produce all the necessary output, read it ourselves with our very own eyes/ears, and then type out the results by hand... but that is definitely *not* what we are going to do! **We want to limit potential sources of errors such as manually typing numbers.**

Instead, we'll look at a couple options for reporting the results dynamically. (We'll come back to the R^2^s and *F*s for each model in a moment.).
...
Finally, for the purposes of reporting results in a paper, we should round this number to make it more readable. As we want **the code to produce the results exactly as they appear in the paper**, we will [use the `round()` function](01_02_intro.qmd#using-functions) to round to two decimal places.
...
We now have a bit of R code that produces a **single** number that we want to report in the text[^2]. The issue is that code chunks contain code, and body text contains text, but we would like the code that we've written to print out its value in the text!

[^2]: Generally, inline code should only ever produce a single value, otherwise the formatting can get *interesting*. This value, however, could be longer than a single number, if you want to get creative with your dynamic reporting!
...
Inferential conclusions are arguably the most important results in a paper, and should also be computationally reproducible just like the numerical results they are based on. Decisions related to whether you should reject or accept a given hypothesis should be based on a quantifiable rule that a computer is able to evaluate (see [Lakens & DeBruine, 2021](https://journals.sagepub.com/doi/full/10.1177/2515245920970949)). One familiar example for people using null hypothesis statistical testing is to reject the null hypothesis if the p-value is smaller than some pre-specified cut-off, such as .05.

So, let's evaluate if the result we computed above is significant or not. If significant, the last sentence in our results paragraph should say:
...
::: {.callout-note appearance="minimal" title="Exercise"}
Another potential option for automatic reporting is the `report()`function from the {`report`} packages (itself part of `easystats`). Check out what `report` does, and use it to report the results of `eval_lm` model. 
...
Next, we will jump to the "Final Model" heading and have a look at how to turn our final model output into a nicely formatted table. It may be tempting to manually create a table in the word processor of your choice and type in the numbers yourself. But we will of couse avoid doing this because typos are way, way too easy to make.  


Once again, {papaja} provides a quick and beautiful solution for reporting, so let's use it again.
...
::: {.callout-note collapse="true" title="Solution"}
Finally, add a note along with the table explaining what the symbols could be presented next to the p-value and what they mean. 
...
For these tutorials, we will generally stick to HTML, as it's the most painless of the rendering options. However, you will likely find yourself wanting to produce some other type of document, which you can easily[^6] do from the same Quarto document.

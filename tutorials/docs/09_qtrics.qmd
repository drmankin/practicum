...
```{r}
## no "s" - returns a single specified label
...
```{r}
labelled::val_label(mil_data$demo_gender, 3) |> 
  gsub(pattern = "^(.*?)\\s.*", replacement = "\\1", x = _)
```
...
- The string `\\s.*`, which is composed of the match for whitespace `\\s` and then the same `.*` wildcard combo we've already seen. Here I've used `\\s` because a literal space ` ` is difficult to spot! The space is critical because it's matching the space between "Other" and the rest of the string in order to separate them.

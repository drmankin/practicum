...
:::
::: {.repro-body}
R packages get regular updates. Some updates are minor but others can change how functions work, and in some cases functions can even be removed. Because of this, it is a good idea to note the specific versions of the packages you are using. One way to do this is to load all the packages you need with `library()` and then call the function `sessionInfo()` to see the specific versions you are using. This functions can generate a lot of output. The most important information is the version of R you are using and the versions of the packages you loaded, which are listed under "other attached packages".
:::
::::
...
[^devil]: I'm playing the devil's advocate a bit here!
...
# Compare the means of the active group and the fist group using a t-test assuming that they have equal variances 

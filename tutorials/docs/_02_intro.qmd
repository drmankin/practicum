...
:::
::: {.repro-body}
R packages get regularly updated. Some updates are minor but others can change how functions work, and in some cases functions can even be removed. Because of this, it is a good idea to note the specific versions of the packages you are using. One way to do this is to load all the packages you need with `library()` and then using the function `sessionInfo()`. This functions can generate a lot of output. The most important information is the version of R you are using and the versions of the packages you loaded, which are listed under "other attached packages".
:::
::::
...
:::
::: {.repro-body}
When using a function, check what the default value for its different arguments are. **The defaults of a given argument may not be what you expect it to be, and that can affect your results.** For example, by default, SPSS computes a type III ANOVA, while the `aov()` function in R computes a type I ANOVA. <!-- I need to double check that this is correct.-->
...
Use the `round()` function to round `r mean_to_round` (stored in `mean_to_round`) to two decimal places.
...
```{r}
# compare the two vectors with a t-test
t.test(vector_1, vector_2, alternative = "less", mu = 5, paired = FALSE, var.equal = TRUE, conf.level = 0.78)
```
...
1. Use explicit style, e.g. `weights::rd()`, when writing our command. In this case we don't need to load the package as long as we have it installed.
...
::: {.repro-body}
Now that we have written some code, it is a good time to introduce comments - one of the best ways to glam up a wall of intimidating code into readable text. A comment is a line or piece of text that R will ignore when running code, and R uses the '#' symbol to signify the beginning of a comment: 
...
Now, for the final exercise for today, go back to the code your wrote above and add some comments in plain English. 
:::

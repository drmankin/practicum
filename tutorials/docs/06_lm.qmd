...
To prepare for our analysis, we will need a place to load packages and read in the data. Any executable code - that is, code that you want to run and do something - must be written in a code chunk and NOT in the body text[^inline].

[^inline]: Mostly true, except for [inline code](#inline-code)!
...
First, let's generate the output we want to report. 
...
We now have a bit of R code that produces a **single** number that we want to report in the text[^2].

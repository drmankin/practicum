...
::::: {.repro-icon}
:::::
RepRoducibility: Object Names
::::

:::: {.repro-body}
You *can* name objects almost anything in R, but that doesn't mean that you *should*. Instead, assume that someone else - a collaborator, a future PhD student, a reproducibility officer, or even your own future self - will be trying to puzzle out what you've done and how. Name objects clearly and sensibly will be a huge boost for this.

When deciding what to name your objects, keep the following guidelines in mind:

1. **Be consistent**. Use consistent separators (i.e. `_`, `.`, etc.) and naming conventions. For instance, all objects containing counts might end with `_n`, or all linear models end with `_lm`. Avoid situations like having the overall number in your sample in `overall_n`, the number in the experimental group in `num_exp`, and the number in the control group in `ctl_count`!

1. **Be transparent**. There's a tradeoff here with succinctness for sure,

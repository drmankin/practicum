...
In this series of tutorials we will learn the basics of R and how to write reproducible code. Computational reproducibility refers to the ability to get the same results each time you run the same analysis on the same dataset. Studies have found that across research fields, only about a quarter to a third of published research papers are computationally reproducible (for examples see: Baker, 2016; Stodden et al., 2018; Crüwell et al., 2023). 
...
1. **Meaningful** – convey information about the information stored in the object
2. **Understandable** - use full words; abbreviations are hard to understand or remember
3. **Concise** - don't use too many words
4. **Formatted** the same way
...
# Names abbreviated beyond comprehension:
...
If you run each line sequentially, the object “monday_quiz_t_test” will be different from the object “tuesday_quiz_t_test” and the results you get for both will be correct. This is because the objects `quiz_9am` and `quiz_6pm` overwritten with the Tuesday marks before the *t*-test for the Tuesday data is run.
...
Compare these results to above. The results in “monday_quiz_t_test” changed and are no longer correct because we used the Tuesday set of data. 

...
Studies have found that across research fields, only about a quarter to a third of published research papers are computationally reproducible (for examples see: [Baker, 2016](https://www.nature.com/articles/533452a); [Stodden et al., 2018](https://doi.org/10.1073/pnas.1708290115); [Crüwell et al., 2023](https://doi.org/10.1177/09567976221140828)). 

This is bad. 
:::

Analyses using point-and-click software can be reproducible in theory, but (in our experience) that can take a long time to reproduce, the process is painful, and the opportunities for mistakes are everywhere. Completing your analyses in a code-based software like R is a step in the right direction, but it doesn’t automatically mean that a code-based analysis is reproducible. In this course, we will learn some of the common pitfalls that can make code not reproducible and help you pass [Reny’s reproducibility certification](https://osf.io/dr35v/) with ease and flair. 
...
1.  [Installing/updating packages](02_intro.qmd)
...
As you can see from the indices, these are three separate commands, because the numbered indices start over from `[1]` each time. However, we want all those numbers *in a single command*. To do this, we'll use a **function** called `c()`.
...
Factors are an essential way to store and label categorical values in R. In a few weeks, we will have a look at how using factors or not can change the way your analyses run. For now, you only need to be aware that factors exist.

::: callout-tip
Factors are so common and useful in R that they have a whole {tidyverse} package to themselves! You already have {forcats} installed with {tidyverse}, but you can [check out the help documentation](https://forcats.tidyverse.org/) if you'd like to learn more about working with factors.
:::
...
Here R prints out a vector of `TRUE`s or `FALSE`s, one for each element in the original vector. So, the first element in the output (`TRUE`) corresponds to the statement `2 <= 6`, the second to `3 <= 6`, and so on. This is a vectorised calculation again, as we saw with numeric data before. These vectorised assertions will be absolutely essential to [selecting and filtering data](04_filter.qmd) that meet particular requirements, or checking our data to find problems.
...
We haven't thus far talked about a few common data types that you might be used to using in SPSS. R also has date-time data, which is not covered in this series, but if you need to use it, [check out the {lubridate} package](https://lubridate.tidyverse.org/).
:::
...
1. **Meaningful**: Convey information about what is stored in the object
2. **Understandable**: Use full words; abbreviations are hard to understand or remember
3. **Concise**: Keep names short enough to type quickly and accurately
4. **Consistent**: Use the same conventions - like order of elements and separators - for all similar objects.
...
quiz_marks_6PM_practical <- c(45, 90, 27, 65, 39, 77)
```
:::
::::
...
To get some practice with creating and working with objects, let's get some data to work with.
...
quiz_t <- t.test(quiz_9am, quiz_6pm)
```

::: {.callout-tip title="Analysis Results"}

`r report::report(quiz_t)`.
...
Let’s say we that the quiz marks we compared were from practicals running on Monday. We also have some data from practicals running on Tuesday, and we want to repeat the exact same analysis on this new data[^stats]. We will use the same variables names for the morning and afternoon practicals across the two days:

[^stats]: We will ignore whether this is an appropriate thing to do statistics wise!
...
Compare these results to above. **The "Monday" object, `monday_quiz_t_test`, now contains the results for the Tuesday data.**
...
# Compare the marks across the two sessions
tuesday_quiz_t_test <- t.test(quiz_tuesday_9am, quiz_tuesday_6pm, var.equal = TRUE)
```

Here, instead of overwriting the `quiz_9am` and `quiz_6pm` objects with new information, we've named all of the objects containing scores with unique names. Now we no longer have to wonder which score information we are using; rather, our `t.test()` functions only work with exactly the right data.

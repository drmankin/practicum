...
> To begin, `` `r knitr::inline_expr("r rep_excl(exclusion_summary, "no_consent")")` `` cases were removed who did not consent. Next, `` `r knitr::inline_expr("r rep_excl(exclusion_summary, "age_young")")` `` cases were removed who were younger than 18, which...
...
> `` `r knitr::inline_expr("r report_all_exclusions(exclusion_summary)")` ``
...
anx_data |>   # <1>
  dplyr::group_by(stars_test_cat = dplyr::case_when(
      stars_test_score > median(stars_test_score, na.rm = TRUE) ~ "high",
      stars_test_score <= median(stars_test_score, na.rm = TRUE) ~ "low",
      .default = NA
    )) |>   # <2>
...
3.  Instead of using the familiar `name = instructions` format, we're instead using `dplyr::across()`. Generally, `across()` has two arguments.
    1.  The first selects which variables to use using `<tidyselect>`. In this case, I've selected all of the variables that contain "score", which will be our subscale composites for the STICSA and the STARS.
    2.  The second provides a list of function(s) to apply to all of the selected variables. So, I've put in the list I made in the previous step that contains all the functions I want to use.

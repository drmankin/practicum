...
::: {.callout-not title="Codebook" collapse="true"}
```{r}
#| echo: false
...
```{mermaid}
flowchart LR
      X -- "`Path *a*`"--> M[Mediator: STICSA Pre-Test State Anxiety] -- "`Path *b*`"--> Y
      X[Predictor: R-MARS Numerical Anxiety] -- "`Total Effect *c*`" --> Y[Outcome: MCQ score]
```
...
After the slight madness of mediation, we're on safe ground again with moderation. As we began this tutorial, we'll end with a nice, straightforward linear model, with minimal bells and whistles. The only fancy bit is the interaction, which we've already had a few examples of.

For this analysis, we'll predict MCQ score from STARS interpretation anxiety score and STICSA trait anxiety score.

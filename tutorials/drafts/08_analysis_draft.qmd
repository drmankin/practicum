...
```{r}
contrasts(anx_scores_data$gender) <- contr.sum(3)
```
:::
:::
...
anx_scores_data <- anx_scores_data |> 
  ...
  dplyr::mutate(
    sticsa_cat = ifelse(sticsa_trait_score > 2, "anxious", "not anxious")
  )
```
...
```{r}
# fit the model:
mcq_afx <- afex::aov_4(sticsa_post_state_score ~ sticsa_cat*mcq + (1|id), data = anx_scores_data)
...
afex::afex_plot(mcq_afx, "mcq", "sticsa_cat") +
  labs(x = "Type of MCQ Test", y = "STICSA Post-Test State Anxiety") +

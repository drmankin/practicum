...
In second year, UGs are also introduced to the (slightly more {tidyverse}-friendly) function `correlation::correlation()`.

On the good side, this function scales up to pairwise tests between as many variables as you give it. This means if you want, for instance, multiple pairwise correlations within a dataset, this is the way to go, since it will apply a familywise error rate correction by default (Holm, to be precise). On the other hand, it's a right pain to type and doesn't play ball with {papaja}, so pick your poison I guess 🤷[^ugh]

[^ugh]: Look, I like my basic {stats} functions. They all look the same and work the same way. They all contain the same things. They all play nicely with {papaja} or custom functions for easy reporting. You don't have to install anything new - if you're using R, they're there. I love an `htest` object, me! But you gotta find the functions that do what you need them to do. You won't necessary always use one or the other - just use the one that makes sense to you and works for the task at hand.

...
1. A logical assertion about the variable(s) in `dataset_name` that returns logical (`TRUE` or `FALSE`) values.
...
2.  Remove the cases and overwrite the old dataset with the new one\[\^overwrite\].
...
For this, we're going to leave `filter()` for a moment at look at a different function, `tidyr::drop_na()`. This function takes a tibble as input, and returns the same tibble as output with any rows that had missing values removed.
...
This makes the counting a bit tricky, so I'm not going to include it as a step in the data cleaning sequence above; but it's useful to know about sometimes.
...
[^lol]: Have you seen the size of this thing?? Of course it isn't!
...
-   `where()` for [selecting with a function](06_filter.qmd#using-functions), described below.
...
The function in `where()` that determines which columns to keep doesn't have to be an existing named function. Another option is to use a "purrr-style lambda" or formula (a phrase you may see in help documentation) to write our own criteria on the spot. What we are essentially doing is writing an ad-hoc, or "anonymous" function - that is, a function that doesn't have a set name.
...
Open the help documentation for `cor.test()`.
...
Both `t.test()` and `cor.test()` offer the option to specify the analysis you want to perform as a **formula**. This is exactly like the formula we saw previously with `lm()`. The format is slightly different but the idea is the same, so let's give it a try.

::: {.callout-note appearance="minimal" title="Exercise"}

Use the formula option for the `cor.test()` function to perform a correlation analysis between any two numeric variables of your choice in the `peng_dat` dataset.

*Hint*: See the "Arguments" section of the help documentation for help with the formula.

...
::: {.callout-tip title="{ggplot2} and Data Visualisation Resources"}
...
Data visualisation with {ggplot2} is explicitly introduced near the end of the first term of Year 1 and practiced throughout the second term. In Year 2, there is less focus on constructing plots from scratch, and instead students are taught how to get a basic plot pre-made and then modify it using their {ggplot2} chops.
...
Load the {tidyverse} and {ggrain} packages.
...
To build a visualisation in {ggplot2}, it's a good idea to build your plot in the same way, from big picture to small detail. As we'll see, I usually build plots layers like this:
...
[^2]: Yes, this is arguably not *technically* true because the values can only be whole numbers between 0 and 5, but what matters here is that *R* thinks this is continuous data.
...
If you have a look at the argument to `breaks` and `limits`, you can see why we took some time early on learning about numeric vectors. `breaks` requires a vector of values indicating where the breaks should be, and `limits` requires a vector of two values, one for the minimum limits and the other for the maximum limit.
...
The `stars_help_cat` variable on the *x*-axis has automatically ordered the categories in alphabetical order, "high" and then "low". As I am a native speaker of a language that reads left to right, I'd prefer to have "low" on the left and "high" on the right[^snarc]. So, how can I change the order of these categories?

[^snarc]: See e.g. [Shaki and Fischer, 2008](https://www.sciencedirect.com/science/article/pii/S0010027708000978?casa_token=6uU795RgZ4wAAAAA:NopLcy7IaTM1YyHXQZ7d3YSjDdV0-WOBJ208crznaivMxdR4AaUtXDMIgXvMZ0LvaRhwyTEBdg)

#### Option 1: Scale Labels

We've seen the `scale_*()` functions already - this time, we have a discrete *x*-axis variable, so we need `scale_x_discrete()`. We've even already seen the `limits =` argument already to set the upper and lower bound of the plot. Here, we can use this argument to specify the order we want the order of the categories to be displayed in instead.
...
We haven't covered factors much in this course so far; for our purposes, most of the time, R treats character data more or less the same way it would treat factors so it hasn't been a problem thus far. However, converting our `stars_help_cat` variable to a factor would let us specify an order of factor levels.
...
anx_scores_data |> 
  dplyr::filter(gender %in% c("male","female")) |> 
  dplyr::mutate(stars_help_cat = factor(stars_help_cat, levels = c("low", "high"))) |> 
  ggplot(aes(x = stars_help_cat, fill = gender)) +
  geom_bar(position = "dodge") +
  papaja::theme_apa(box = TRUE)
```
...
anx_scores_data |> 
  ...
  scale_x_discrete(name = "STARS Asking for Help Anxiety",
                   ## This sets the order using the values in the variable
                   limits = c("low", "high"),
                   ## This sets the labels to appear in the plot
                   labels = c("Low", "High")) +
  ## I think this looks good but you can do what you like!
  scale_y_continuous(name = "Count",
                     limits = c(0, 175),
                     breaks = seq(0, 175, by = 25))+
  ## More Sussex colours :)
  ## With a colourblind-friendly and greyscale-print-resilient colour scheme
...
Keep in mind that `ggplot()` is just applying the labels as strings on top of your plot. This means that if you, for instance, reorder your categories on the *x*-axis but then forget to update the order of axis labels in your code, R will not stop you! It's always important to double-check that the labels you add do in fact correspond correctly to the order of the categories in the plot to avoid mislabeling.

As an example, the code below is identical to the solutiona above, except I've commented out the `limits` argument in `scale_x_discrete()` that reorders the categories on the *x*-axis. However, I've left in the `labels` argument that whacks on the axis labels in the reordered order. Comparing to the plot above, we can see by the sizes of the bars that the actual categories are in high-low order, despite what the nicely formatted labels say.
...
4.  Adjust the discrete *x* axis by giving it a name, and relabelling the group labels, *plus* 5 Adjust the continuous *y* axis by giving it a name, *plus*
5.  Add an APA-style theme.
...
3.  Draw a raincloud plot, *plus*
4.  Adjust the discrete *x* axis by giving it a name, and relabelling the group labels, *plus* 5 Adjust the continuous *y* axis by giving it a name, *plus*
5.  Calculate a summary of the data using the function `mean_cl_normal` for means and CIs, and add to the plot in red, *plus*
...
anx_scores_data |> 
  ggplot(aes(x = mcq, y = stars_test_score, fill = mcq)) +
  ## Customise geom_rain points to be transparent (alpha) and coloured by mcq group
  geom_rain(point.args = list(alpha = .4, aes(colour = mcq))) +
  ## Change the shape of the means
  stat_summary(fun.data = mean_cl_normal,
               shape = 23) +
  scale_x_discrete(name = "MCQ Condition", labels = c("Maths", "Stats")) + # <4>
  scale_y_continuous(name = "Mean STARS Test Score") +
  ## Specify custom colours for the fill and colour elements
  scale_fill_manual(values = c("#f2c75c", "#007a78")) +
  scale_colour_manual(values = c("#dc582a", "#003b49")) +
  ## Remove the legend
  guides(fill = 'none', color = 'none') +
  ## Flip the coordinates - reverses x and y!
  coord_flip() +
  papaja::theme_apa(box = TRUE)
```
:::
...
::: {.callout-tip title="What about Interaction Plots?" collapse="true"}
You might be wondering about the classic interaction plot, very common in factorial and mixed designs, depicting the means in each combination of groups of a categorical variable, like this:

```{r}

```

We will cover these plots, but in the next tutorial, when we will cover the analyses that these visualisations would typically accompany.
:::
...
To start, let's set up a basic plot using
...
4.  Adjust the continuous *x* axis by giving it a name, and setting the limits to 1 and 4, *plus* 5 Adjust the continuous *x* axis by giving it a name, and setting the limits to 1 and 4, *plus*
5.  Add an APA-style theme.
...
anx_scores_data |> # <1>
  ggplot(aes(x = sticsa_pre_state_score, y = sticsa_post_state_score)) + # <2>
  geom_point(alpha = 0.4, position = "jitter") +  # <3>
  geom_smooth(method = "lm", colour = "darkblue", fill = "darkblue") + # <4>
...
### Colour and Fill Auto

In the [previous Colour and Fill section](07_dataviz.qmd#colour-and-fill-manual) we chose colours by name or hex code to add to a plot. However, there are a wide variety of pre-made colour palettes to make use of. So, instead of chosing individual colours, you can quickly apply existing colour palettes.
...
A popular palette and the one we'll use for this task is [the lovely {viridis} colour palette](https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html), designed to be pretty, print-friendly, and accessible (robust to colourblindness).
...
```{r}
anx_scores_data |> # <1>
  ggplot(aes(x = sticsa_trait_score, y = sticsa_post_state_score,  # <2>
             colour = mcq, fill = mcq)) + # <2>
  geom_point(alpha = 0.4, position = "jitter") +  # <3>
...
[^3]: Naturally, if you want to use palettes from a different package, you'll need to use the functions from that package instead! Those functions may also work a bit differently, so make sure you read the help documentation if you get stuck.
...
anx_scores_data |> # <1>
  ...
  scale_y_continuous(name = "STICSA Post-MCQ State Anxiety Score", # <6>
                    limits = c(1, 4)) + # <6>
  scale_colour_viridis(name = "MCQ Type", discrete = TRUE, labels = c("Maths", "Stats")) + # <7>
  scale_fill_viridis(name = "MCQ Type", discrete = TRUE, labels = c("Maths", "Stats")) + # <7>
  papaja::theme_apa(box = TRUE) # <9>
```
...
```{r}
mcq_lm_plot <- anx_scores_data |> # <1>
  dplyr::mutate(
    `MCQ Type` = stringr::str_to_title(mcq)
  ) |> 
  ggplot(aes(x = sticsa_trait_score, y = sticsa_post_state_score,  # <2>
             colour = `MCQ Type`, fill = `MCQ Type`)) + # <2>
  geom_point(alpha = 0.4, position = "jitter") +  # <3>
  geom_smooth(method = "lm") + # <4>
  scale_x_continuous(name = "STICSA Pre-MCQ State Anxiety Score", # <5>
                    limits = c(1, 4)) + # <5>
  scale_y_continuous(name = "STICSA Post-MCQ State Anxiety Score", # <6>
                    limits = c(1, 4)) + # <6>
  scale_colour_viridis(discrete = TRUE) + # <7>
  scale_fill_viridis(discrete = TRUE) + # <7>
  papaja::theme_apa(box = TRUE) # <9>
...
Finally, the last function creates an interactive widget for the plot.
...
<!-- We encountered the delightful {GGally} package in the previous tutorial for producing complex, information-rich correlation matrices with minimal effort. -->
...
<!--   dplyr::select(!age & where(is.numeric)) |>  -->
...
In a previous section, we stored our lovely scatterplot as `mcq_lm_plot`. Let's see now how we could adjust the way that figure would appear in a Quarto document.

To begin, we're just going to put our plot object in a new code chunk, in the document where we want it to appear. Equally, this could be the code that produces the plot; it doesn't matter either way.
...
<!-- ::: {.callout-warning "Code Watch: YAML vs R"} -->

<!-- You may notice some clear differences between R code, and the way we write code chunk options. This is because code chunk options are  -->
...
To include more than one figure in the same group, we can use the `layout-nrow` and `layout-ncol` options to decide how these appear. For `layout-nrow: 2`, there will be two *rows* in the output, so the figures will appear one above the other:

```{r}
#| layout-nrow: 2

anx_scores_data |> 
  ggplot(aes(x = stars_test_score, y = stars_m_test_score)) +
  geom_point(alpha = .4, position = "jitter") +
  geom_smooth(method = "lm") +
  papaja::theme_apa(box = TRUE)

anx_scores_data |> 
  ggplot(aes(x = rmars_test_score, y = rmars_s_test_score)) +
  geom_point(alpha = .4, position = "jitter") +
  geom_smooth(method = "lm") +
  papaja::theme_apa(box = TRUE)
```

Whereas for `layout-ncol: 2`, there will be two *columns* in the output, so the figures will appear side by side:
...
To really bring it all together, we could define a custom layout and add captions and sub-captions, with the sub-captions ordered in the same order as the plots.

```{{r}}
#| layout: [[45,-10, 45], [100]]
#| fig-cap: "Scatterplots and correlogram"
#| fig-subcap: 
#|   - "Scatterplot of STARS and STARS-M Test subscale"
#|   - "Scatterplot of RMARS and RMARS-S Test subscale"
#|   - "Correlogram of all four Test subscales"
...
anx_scores_data |> 
  dplyr::select(contains("test")) |> 
  GGally::ggscatmat(alpha = 0.4) +
  labs(x = "Mean Score (1-5)", y = "Mean Score (1-5)") +
  papaja::theme_apa(box = TRUE)
```

...
Welcome to the first Sussex Psychology Staff Training tutorial! Let's jump right in and get started.

This tutorial is designed to accompany a "Workbook" document, which, if you are joining the live training course, is already available for you on the Posit Cloud workspace. To access it, navigate to [Posit Cloud](https://posit.cloud) and open the course workspace. If you haven't joined the workspace yet, use the join link on the [Posit Cloud page on Canvas](https://canvas.sussex.ac.uk/courses/29761/pages/posit-cloud).
...
We will eventually work with all four of these windows, but once you've opened the workbook, you can **ignore the Environment and Files windows** for the time being. We'll be focusing only on the other two: Source and Console.
...
You can use the body text portion of a Quarto document more or less like you would a document in Microsoft Word (or your word processor of choice). In the body of the document, you can write and format any text you want - notes, questions, thoughts, ideas, comments, etc. This workbook already contains all the headings from this tutorial to help you get organised, but please do delete or edit as you see fit.
...
2.  Opening help documentation
3.  Drafting or testing code to go into a document.
...
If there's one thing that's for sure about working in R, it's this: you will, inevitably, make typos and errors using R. You will write commands that make sense to you that R doesn't understand; and you will write commands that **don't** make sense to you, that R **does** understand. Errors are an essential and unavoidable part of learning R, so let's start there.

::: {.callout-note appearance="minimal" title="Exercise"}
Type literally any gibberish, words, keysmashes etc. into the code chunk in the workbook and press the green Run button (or Ctrl/Cmd + Shift + Enter).

::: {callout-tip}
The `{r}` at the top of the code chunk has to stand on its own. Any code you write must start on the line underneath it.
:::
...
If you haven't tried this yet, and your pristine document is just ominously staring at you, I'm serious - punch your keyboard if you have to, or let your cat walk on it, or play it as if it were a piano, and run the code. There's two important things to learn from this:
...
Very often, R communicates with you via errors. Unlike many other computer programmes you might be familiar with, an "error" in R doesn't (usually!) mean a catastrophic failure and/or potential loss of hours of work[^2]. Rather, whatever you've asked R to do, it's essentially replied, "[Sorry, I can't do that.](https://youtu.be/7qnd-hdmgfk?feature=shared)" Two of the most important skills you can develop early on with R is to **treat errors as feedback**, and to **learn to read and recognise errors**.
...
::: callout-tip
You can run all the code in a code chunk by pressing Ctrl/Cmd + Shift + Enter on your keyboard, or by clicking the green "play" arrow in the top right corner of the code chunk.
...
::: {.callout-tip title="MoRe About: Spotting Problems" collapse="true"}
...
This example also shows us something useful: you can run multiple commands within the same code chunk. While spaces are not meaningful to R (that is, `40 - 8` and `40-8` and `40      -8` are all produce the same output), new lines have an important role to play separating out commands. Each command must have its own new line.

#### Vectors {#vectors}

Imagine you are collecting data for a study, and you want to generate some simple participant ID numbers to keep track of the order that they completed my study. You had 50 participants in total. You could do this by typing every number out one by one, but this is exactly the kind of tedious nonsense that R is great at. Instead, we'll use the operator `:`, which means "every whole number between".
...
This is our first contact with functions in R, and [we'll explore how they work more later](02_functions.qmd). To use this one, type it out, then inside the brackets, put the numbers you want to **c**ollect (or **c**oncatenate, or **c**ombine), with different sequences separated by commas.
...
As you can see from the numbered indices this time, R **c**ollects all of the numbers into a single series of elements, called a **vector**.

Actually (surprise!), we've been looking at vectors this whole time. Just about any series of single pieces of information in R is a vector (but see Tip box on vectors and elements). When we were looking at single numbers (like `3958` above), we were still getting a vector back from R, but it was a vector with only one element, and thus only `[1]`.
...
What I've essentially asked R is, "Put all of these numbers into a single vector, and then give me the 33rd element in that vector." As it turns out, the 33rd element in that vector of numbers is `r c(12:30, 23:55, 36)[33]`.
...
::: {.callout-note appearance="minimal" title="Exercise"}
Write and run the following assertions in R:
...
Coersion is when data is forcibly changed from one type to another. This is sometimes intentional, but it can happen unintentionally (and without any warning or fanfare!), so is a common source of errors.
...
[^3]: Well, not in the single command we're using here. We can certainly get out the age `36`, but it will take a bit more work. We'll come back to this problem in the Essentials section of the course.

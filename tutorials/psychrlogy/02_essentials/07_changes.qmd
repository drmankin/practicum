...
Similar to [what we saw with `rowwise()`](05_changes.qmd#composite-scores), `group_by()` creates internal structure in the dataset - a new group for each unique value in the grouping variable. Any subsequent calculations done with the dataset are done within those groups.
...
1.  Take the dataset, and then group by the values in the `anxiety_cat` [variable that we created earlier](05_changes.qmd#one-variable-input).
...
::: callout-tip
For more explanation about `dplyr::across()`, see the [section on iteration with `mutate()`](05_changes.qmd#iteration) earlier on. For a much more in-depth explanation, run `vignette("colwise")` in the Console.
:::
...
The [next tutorial](06_dataviz.qmd) applies everything we've covered so far to the creation of beautiful and professional data visualisations.
...
-   Using `<tidyselect>`, see [Tutorial 04: Filter and Select](04_filter.qmd)

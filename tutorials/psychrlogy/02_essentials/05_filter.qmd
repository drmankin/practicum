...
The first thing you might think to try is to filter on `sticsa_trait_3 == NA`, but weirdly enough this doesn't work. Instead, we again need the increasingly versatile `is.na()`, which again, we can think of `is.na()` as a question about whatever is in its brackets: "Is (this) `NA`?" Let's see this in action:

...
Today we're going to start working with a dataset that we're going to get familiar with over the next few weeks. Courtesy of the fantastic Sussex colleague (Jenny Terry)[https://www.jennyterry.co.uk/], this dataset contains real data about statistics and maths anxiety. 
...
2.  Filter it keeping only the cases where the following assertion is true:
3.  The value in the `mcq` variable is exactly and only equal to `"maths"`.

So, the tibble we get as output contains cases that have the value `"maths"`, and NOT `"stats"`, nor any `NA`s (because `NA` does not equal `"maths"`!).
...
1.  Take the dataset `anx_data`, *and then*
2.  Filter it keeping only the cases where the following assertion is true:
3.  The value in the `age` variable is less than 40.
...
As a final example, let's consider a situation where we want to retain only participants that gave a gender identity of either "male" or "female".[^gender]

[^gender]: I'm not wild about this example - the experiences of non-binary and other genders are just as important! Unfortunately it's the only variable in the dataset with the right number of categories.
...
anx_data |> #<1>
  dplyr::filter( #<2>
    gender %in% c("female", "male") #<3>
    )
```

1.  Take the dataset `anx_dat`, *and then*
2.  Filter it keeping only the cases where the following assertion is true:
3.  The value in the `gender` variable matches any of the values "female" or "male".
...
1.  Take the dataset `anx_dat_proc`, *and then*
2.  Filter it keeping only the cases where the following assertion is true:
...
So, to create my clean `anx_dat_final` dataset, I can use the assertion `!is.na(sex)` to keep only the penguins who do **NOT** have `NA` in the `sex` variable.
...
```{r}
anx_dat_final <- anx_dat_proc |> #<1>
  dplyr::filter( #<2>
    !is.na(sex) #<3>
  )

n_final <- nrow(anx_dat_final)
n_final
```
...
There's a huge advantage of this, namely *ease of change*. Imagine you had a collaborator join the study, with records from more Adelie penguins to use. In order to update all your numbers, all you have to do is update your initial `anx_dat` dataset with the new cases, and then re-run all your code as is. Because these objects count whatever is in the data, they will automatically contain and record the correct numbers for the data you put into them.
...
::: {.callout-note appearance="minimal" title="Exercise"}
Create a subset of `anx_dat` that contains the following variables:
...
-   `where()` for [selecting with a function](05_filter.qmd#using-functions), described in the next section
...
Select the variables in `anx_dat` that have to do with bill measurements.
...
Select the variables in `anx_dat` that do NOT contain measurements in mm.
...
::: {.callout-note appearance="minimal" title="Exercise"}
**CHALLENGE:** Select the variables in `anx_dat` that are factors, or that do NOT contain any missing values.
...
::: {.callout-note appearance="minimal" title="Exercise"}
Select at least three numeric variables from `anx_dat` and pipe into `ggscatmat()`.

...
[^2]: Yes, this is not *technically* true because the values can only be whole numbers between 0 and 5, but what matters here is that *R* thinks this is continuous data.
...
::: {.callout-warning}
*Note*: This gets a bit into the weeds, so if you'd like to skip this section, feel free to skip down to [the next type of plot(07_dataviz.qmd#barplots).
:::
...
To start, let's set up a basic plot using the STICSA pre- and post-MCQ state anxiety scores. The new element is `geom_point()`, which draws points [^scatter]. In that layer, we also have two new arguments. The first is `alpha`, which sets the transparency of the points between 1 (solid) and 0 (invisible). The second is `position = "jitter"`, which introduces some random noise into the placement of the points so make it easier to see overlapping values.

[^scatter] Tragically, `geom_scatter()` isn't a thing, not matter how many times I try!
...
anx_scores_data |>
  ggplot(aes(x = sticsa_pre_state_score, y = sticsa_post_state_score)) + # <2>
  geom_point(alpha = 0.4, position = "jitter") +  # <3>
  geom_smooth(method = "lm", colour = "darkblue", fill = "darkcyan") + # <1>
...
```{r}
anx_scores_data |>
  ggplot(aes(x = sticsa_trait_score, y = sticsa_post_state_score,  # <1>
             colour = mcq, fill = mcq)) + # <1>
  geom_point(alpha = 0.4, position = "jitter") +  # <3>
...
1. Set up the plot, mapping `sticsa_pre_state_score` to the *x*-axis and `sticsa_post_state_score` to the *y*-axis, and both colour and fill to `mcq`.
...
anx_scores_data |> 
  ...
  scale_y_continuous(name = "STICSA Post-MCQ State Anxiety Score", # <6>
                    limits = c(1, 4)) + # <6>
  scale_colour_viridis(name = "MCQ Type", discrete = TRUE, labels = c("Maths", "Stats")) + # <1
  scale_fill_viridis(name = "MCQ Type", discrete = TRUE, labels = c("Maths", "Stats")) + # <1>
  papaja::theme_apa(box = TRUE) # <9>
```

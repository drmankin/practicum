...
::: {.callout-warning}
*Note*: This gets a bit into the weeds, so if you'd like to skip this section, feel free to [jump ahead to barplots](07_dataviz.qmd#barplots).
...
Next, we need to get each of the variables out of the dataset using `$` subsetting. This is *exactly* the same method we used in the very first tutorial to run a correlation! Remember to use the new, filtered dataset rather than the original.
...
anx_scores_data |> 
  ...
  dplyr::count(gender, stars_help_cat) |> 
  chisq.test()
```
...
> A $\chi^2$ test of association revealed no significant association relationship between gender and STARS Asking for Help anxiety (`` `r knitr::inline_expr("chisq_out$full_result[[1]]")` ``).
...
To start, let's set up a basic plot using the STICSA pre- and post-MCQ state anxiety scores. The new element is `geom_point()`, which draws points (tragically, `geom_scatter()` isn't a thing, not matter how many times I try!). In that layer, we also have two new arguments: `alpha`, which sets the transparency of the points between 1 (solid) and 0; and `position = "jitter"`, which introduces some random noise into the placement of the points so make it easier to see overlapping values.
...
anx_scores_data |> # <1>
  ggplot(aes(x = sticsa_pre_state_score, y = sticsa_post_state_score)) + # <2>
  geom_point(alpha = 0.4, position = "jitter") +  # <3>
  geom_smooth(method = "lm", colour = "darkblue", fill = "darkcyan") + # <4>
...
A popular palette and the one we'll use for this task is [the lovely {viridis} colour palette package](https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html), designed to be pretty, print-friendly, and accessible (robust to colourblindness).
...
To begin, we're just going to put our plot object in a new code chunk, in the document where we want it to appear. Equally, this could be the code that produces the plot; it doesn't matter either way. Here, I've stored the final output for the LM plot we created above in a new object, `mcq_lm_plot`, to make the subsequent sections easier to read.

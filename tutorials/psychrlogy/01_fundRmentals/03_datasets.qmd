...
So, we need some data to work with. We're going to start by importing, or "reading in", some data from an outside source. The first job is to work out how that data is stored: the file format that it's in, and the location we need to give to R to look for it.
...
This is, I am sure you will agree, illegible and absolutely horrifying.

:::
:::

From this point forward, we'll start working with the native pipe. The following sections will have specific examples on using the pipe to practice.
...
- `ncol()`: Returns the **n**umber of **column**s as a numeric value.
...
This format takes a bit of getting used to. The new object, which I've called `px_initial`[^px], is created at the first line of the command by the `<-`. However, this object *contains* whatever the final output of this pipe is at the end.
...
Once we've had a look at the whole dataset, it's time to drill down into individual variables. We may want to calculate quick descriptives or investigate what's going on with particular variables that seem to have issues. For any of these tasks, we'll look at variables one at a time by subsetting or otherwise pulling them out of the dataset, then calculating some information about them.
...
Subsetting with `$` is the base-R method. `dataset_name$variable_to_subset`. 

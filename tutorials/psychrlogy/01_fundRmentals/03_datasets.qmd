...
Calling a tibble is a great way to get a snapshot or do a quick check, but for larger datasets, it's less useful - too much clicking and scrolling. Instead, we can have a look at the whole dataset more easily - and interact with it to some degree - by viewing it, which opens a copy of the dataset in the Source window to look through. We can do this with the `View()` function (note the capital "V"!).
...
For functions where this is not the case, you can explicitly determine where the piped-in object should go using a "placeholder". The most noticeable difference is that the native pipe placeholder is `_`, while the magrittr pipe placeholder is `.` - there's an example of this below to help make this clearer.

...
This tutorial is still under construction, so will be updated in the future.
:::
...
One key concept for using R is the different ways it categorises data. "Data" here means any piece of information you put into R - a word, a number, the result of a command or calculation, a dataset, etc. Depending on the type of data you have, R will treat it differently, and some operations only work on certain types of data. So, let's have a look at how R encodes and deals with different types of data. Here's well cover three of the most common and important: numeric, character, and logical. As we do so, we'll practice some core skills in R.
...
::: {.callout-note appearance="minimal" title="Exercise"} {#create-objects}
...
First, let's imagine we get three new participants in each condition of our previous study. Update [the same two objects you created previously](01_02_intro.qmd#create-objects) with three new scores each.
...
quiz_9am <- c(75, 58, 62, 14, 33, 67, 55, 89, 60, 79)
```

However, this is tedious, repetitive, and prone to error. Wherever possible, it's better to rely on R to do calculations or repetitions for you. So, we could instead just embed the previous `quiz_9am` object - which already contains the first seven numbers - into a `c()` along with the three new numbers.

```{r}
quiz_9am <- c(quiz_9am, 89, 60, 79)
quiz_9am

quiz_6pm <- c(quiz_6pm, 38, 75, 54)
quiz_6pm
```
...
**Overwriting objects is accomplished by assigning new output to an existing object name.** If you have a look in your Environment, you will see that the previous version of `quiz_9am`, containing only seven values, has been replaced with the new one containing ten values.

**Overwriting objects is silent.** Unlike, say, a word processer, that will give you a warning if you try to save two documents in the same folder with the same name, R won't ask you if you're sure you want to overwrite an existing object with new information - it will just **do it**. This can be a good thing, because you can easily update the information stored in an object with changes, edits, or new information. However, it also means that you can overwrite or replace data when you don't want to, if you use the same object name.
...
quiz_9am_full <- c(quiz_9am, 89, 60, 79)
quiz_9am_full
...
Default values of arguments are really useful, because the default is often the most frequently used or safest[^4] setting. It means you don't have to specify every single aspect of a function every time you use it, as long as you want the function to work according to its defaults. In our case, we actually wanted `round()` to round to two decimal places, not 0. So, in our command, we should change the `digits` setting from the default, `0`, to `2`.

[^4]: By "safest" setting, I mean that the function makes the fewest assumptions about what you intended.

### Using Functions

Now that we know what both of these arguments mean, we can change them to actually translate the sentence "Round the number `r mean(quiz_6pm)` to two decimal places" into a command that R can work with. We'll explicitly write out each argument so we know what they are doing.
...
```{r}
round(55.76923, 59.54, 0.198, 2)
```

Once again, R tells us that this doesn't work by throwing an error. R has tried to do what we wanted, but the `round()` function only allows a max of two arguments, and we've given it four. Behind the scenes, R has tried to run `round(x = 55.76923, digits = 59.54...` and can't proceed from there because it doesn't know what to do with the last two numbers. So, what we need to do is find a way to put all three numbers that we want to round into the first `x` argument together. If only there was a way to **c**oncatenate them together...
...
numbers <- c(55.76923, 59.54, 0.198)
round(numbers, digits = 2)

## Put the vector of numbers into round() directly
round(c(55.76923, 59.54, 0.198), digits = 2)
```
:::

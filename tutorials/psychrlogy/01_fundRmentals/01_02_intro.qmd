...
Ignore the YAML header for now; we'll come back to Quarto documents, including the YAML, in depth [in Tutorial 03](04_lm.qmd#quarto).
...
If **no**, write the code in the **Console**. Code written directly in the Console isn't saved or documented anywhere[^hist]. Some common uses of the Console are:

1. [Installing/updating packages](03_datasets.qmd)
1. Drafting or testing code to go into a document.

[^hist]: This isn't *technically* true - have a look at the "History" tab in the Environment window. However, the commands stored here can't be run or used - they have to be copied into the Console or Source windows in order to run. The History tab provides an exhaustive record of of the things you've typed, not a cohesive or meaningful series of steps.
...
quiz_9am <- c(75, 58, 62, 14, 33, 67, 55, 89, 100, 79)
```
...
Let's look at an example of how this translation might work. For this example, I'm going to use a number I generated earlier: the mean of the `quiz_9am` group, which was `r mean(quiz_9am)`, which I'd like to round to two decimal places - a common task for reporting results in APA style.
...
::: {.callout-note appearance="minimal" title="Exercise"}
Use the `round()` function to round 69.23077 to two decimal places.
...
```{r}
round(69.23077, 59.54, 0.198, 2)
```

Once again, R tells us that this doesn't work by throwing an error. R has tried to do what we wanted, but the `round()` function only allows a max of two arguments, and we've given it four. Behind the scenes, R has tried to run `round(x = 69.23077, digits = 59.54...` and can't proceed from there because it doesn't know what to do with the last two numbers. So, what we need to do is find a way to put all three numbers that we want to round into the first `x` argument together. If only there was a way to **c**oncatenate them together...
...
numbers <- c(69.23077, 59.54, 0.198)
round(numbers, digits = 2)

## Put the vector of numbers into round() directly
round(c(69.23077, 59.54, 0.198), digits = 2)
```
:::

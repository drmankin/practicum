...
::: {.callout-imporant title="Which Tutorial?"}
...
If you encounter this issue, you may need to [inspect and clean your dataset](..\02_essentials\04_filter.qmd) before you proceed with analysis!
:::
...
1.  The [**YAML header**](03_lm.qmd#global-options) at the top, delineated by `---`s, which contains information about how the document will be *rendered*
2.  The [**body text**](03_lm.qmd#headings-and-text), which contains regular (i.e. non-code) text
3.  The [**code chunks**](03_lm.qmd#code-chunks), which contain code - in this case, specifically, R.
...
[^2]: Mostly true, except for [inline code](03_lm.qmd#inline-code)!

::: {.callout-tip title="Inserting Code Chunks"}
...
-   Create the two objects containing the linear models with [one predictor](03_lm.qmd#one-predictor) and with [two predictors](03_lm.qmd#hierarchical-models).
...
In the Posit Cloud project workspace for this tutorial, there is a `teaching.csv` object. Look back on [Tutorial 02](02_datasets.qmd) for more detail about how to read in datasets.
...
-   They can be automatically converted into a [table of contents](03_lm.qmd#table-of-contents)
...
::: {.callout-tip title="Inserting Headings"}
...
::: {.callout-tip title="Formatting Text"}
...
::: callout-tip
This section will make extensive use of `$` subsetting, which was covered in [Tutorial 02](02_datasets.qmd), and `[]` subsetting, which was covered in [Tutorial 01](01_intro.qmd).
:::
...
Finally, we can [use the `round()` function](01_intro.qmd#using-functions) to round to two decimal places.
...
The solution is *inline code*, a small bit of R code written in the body text ("inline") that will, when the document is rendered, automatically insert the right number. Inline code is written as follows, in the text: `` `r knitr::inline_expr("some_r_code")` ``
...
You can of course write whatever you like, or borrow the text below, but use "@tbl-final-model" (or whatever label your gave the table code chunk) to refer to the table.
...
[^5]: For writing tutorials, I make extensive use of `eval: false`, which includes the code in the output but does not attempt to run the code. This allows me to write all kinds of nonsense code without R getting stroppy and throwing errors all of the place!
